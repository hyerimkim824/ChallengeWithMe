...
public class ChallengeDAO {
	...
	public int getListCount(String keyfield, String keyword, String cate_num, int official) throws Exception {
		...
		try {
			...
			sql = "SELECT COUNT(*) FROM chall c "
			        ...
			        + "JOIN cate cat ON c.cate_num = cat.cate_num WHERE c.official="+official + sub_sql;
	...
	//챌린지 목록
	public List<ChallengeVO> getList(int start, int end, String keyfield, String keyword, String cat_num, int official) throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		String sub_sql = "";
		...
		try {
			...
			if(keyword != null && !"".equals(keyword)) {
				if(keyfield.equals("1")) sub_sql += " AND chall.ch_title LIKE '%' || ? || '%'";
			...
			sql = "SELECT * FROM (SELECT chall.*, cate.cate_name, us.us_nickname, rownum AS rnum"
			        + " FROM chall JOIN cate ON chall.cate_num = cate.cate_num JOIN user_detail us ON chall.us_num = us.us_num WHERE chall.official="+official + sub_sql
